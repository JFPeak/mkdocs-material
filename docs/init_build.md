...
## 安装
Windows系统预先从官网安装好Python3和Git
（一）以管理员身份打开vscode软件，在合适的文件夹内打开终端，在终端中输入：
```shell
# 转入虚拟环境
...
```
## 进一步配置
### vscode安装YamL by Red Hat 扩展插件
用以防止yaml格式错误导致的bug以及查看各key可对应的value等。VS Code左下角⚙️->设置->右上角“打开设置(json)”。编辑settings.json，加入以下：
```json
{
...
...
```
##  使用Git & Github 并部署至github page
### 创建GitHub Actions 的自动化脚本
当前工作目录下创建.github/workflows/ci.yml 文件并粘贴示例代码：
```yml
name: ci
...
```
只要把代码 push 到 main（或 master）分支，脚本就会自动把 MkDocs 生成的静态网站部署到 GitHub Pages，完全不用手动操作。GitHub Pages 默认读取 gh-pages 分支，于是你的网站瞬间上线，地址通常是https://<github用户名>.github.io/<仓库名>/
### 创建GitHub仓库和上传ssh公钥
...
1. （只做一次）进入仓库设置，在Pages页面里更改为deploy from branch，选择gh-pages分支，保存。
2. （只做一次）进入仓库设置，
3. （只做一次）新仓库/新客户端：git init → remote add → branch -M main
4.  日常发文章循环：git add . → git commit → git push

## 结语

至此，我们就完成了一个简洁的技术博客的搭建。后续再对博客配置进行优化。
